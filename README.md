# Databend Links

This repo hosts all useful links of databend.

Click the button below to deploy on Vercel.

[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https%3A%2F%2Fgithub.com%2Fdatabendlabs%2Flink&env=GITHUB_REPOS,GITHUB_TOKEN&envDescription=Environment%20Variables%20Reference&envLink=https%3A%2F%2Fgithub.com%2Fdatabendlabs%2Flink%23environment-variables)
[![Deploy to PandaStack](https://dashboard.pandastack.io/deploy-button.svg)](https://dashboard.pandastack.io/deploy?repo=databendlabs/link&type=static&buildCmd=npm+run+build&outputDir=dist)

## Environment Variables

- `GITHUB_TOKEN`: Github personal access token with read access.
- `GITHUB_REPOS`: Github repositories separated by comma, e.g. `databendlabs/databend,databendlabs/opendal`.

Please report to us via [issues](https://github.com/databendlabs/link/issues) while meeting any link broken.

## Current

- [I'm feeling lucky](https://link.databend.com/i-m-feeling-lucky): Open a good first issue for you!
- [Join Slack](https://link.databend.com/join-slack): Join databend slack to have live communication
- Projects: Tasks that databend community is working on
  - [Databend Query Engine](https://link.databend.com/project-query)
  - [Databend Storage Layer](https://link.databend.com/project-storag)
- [Visit Twitter](https://link.databend.com/twitter): Visit datafuse labs twitter
